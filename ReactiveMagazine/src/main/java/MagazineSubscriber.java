...
public class MagazineSubscriber implements Flow.Subscriber<Integer>  {
    ...
    private Integer lastEditionReceived;
    ...
    public void onNext(Integer item) {
        if(this.lastEditionReceived > 0 && item - this.lastEditionReceived > 1) {
            int lost = this.lastEditionReceived.intValue() + 1;
            this.log("<" + this.subscriberName +  ">Perdi uma edição, recebi " + item +
                    " e a última recebida era " + this.lastEditionReceived + " perdi a edição " + lost);
        }
        this.lastEditionReceived = item;
        this.takeSomeRest();
        this.subscription.request(1);
    }
    ...
    public void onComplete() {
        this.log("<" + this.subscriberName +  "> Assinatura concluida");
    }
